...
pub(crate) fn builtin_presets() -> HashMap<String, Preset> {
    ...
    presets.insert(
        ...
        Preset {
            ...
            persona: "You are JARVIS, a polite British AI butler in the spirit of Iron Man. Speak with calm precision and dry wit. Address the user as \"sir\" sparingly. Keep replies concise; never grovel or pad with filler.".to_string(),
        },
    );
    presets.insert(
        "concise".to_string(),
        Preset {
            ...
            persona: "Be extremely terse. Use short sentences. No preamble, no filler, no apologies. Confirm work with one short phrase, not a recap.".to_string(),
        },
    );
    presets.insert(
        "pirate".to_string(),
        Preset {
            voice: Some("ash".to_string()),
            persona: "Speak as a swashbuckling pirate captain. Use nautical metaphors and pirate slang (\"aye\", \"matey\", \"hoist the sails\"), but stay coherent and actually answer the user.".to_string(),
        },
    );
    presets
}
